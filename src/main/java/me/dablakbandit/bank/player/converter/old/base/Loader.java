...
public abstract class Loader extends SQLListener {
			...
			if (jo.has("" + tab)) {
				...
				for (int ii = 0; ii < ja.length(); ii++) {
					...
					try {
						...
						e.printStackTrace();
						if (jo1 != null) {
							System.out.println("Failed to load item: " + e.getMessage() + " : " + jo1);
						}
						throw e;
					}
				}
		...
		if (jo1.has("display")) {
			...
			if (jo2.has("Name")) {
				...
				try {
					...
					BankLog.error("Fixing name for " + ja2);
					JSONFormatter jf = new JSONFormatter();
					jf.append(s);
					ja3.remove(1);
					ja3.put(jf.toJSON());
					BankLog.error("Fixed name for " + ja2);
				}
			}
			if (isFourteen()) {
				if (jo2.has("Lore")) {
					...
					for (int i = 0; i < ja4.length(); i++) {
						...
						try {
							...
							BankLog.error("Fixing lore for " + ja2);
							JSONFormatter jf = new JSONFormatter();
							jf.append(s);
							ja5.remove(1);
							ja5.put(jf.toJSON());
							BankLog.error("Fixed lore for " + ja2);
						}
					}
				}
			}
	...
	protected final void fixEnchantments(JSONArray ja) throws Exception {
		for (int i = 0; i < ja.length(); i++) {
			...
			if (jo.has("id")) {
				...
				if (ja1.getInt(0) == 2) {
					BankLog.error("Fixing enchantment for " + jo);
					int id = ja1.getInt(1);
					...
					if (ef != null) {
						...
						BankLog.error("Fixed enchantment for " + jo);
					} else {
						BankLog.error("Unable to fix enchantment for " + jo);
					}
				}
			}
		}
	...
	protected final ItemStack convertJSONToItemStack(JSONObject jo) throws Exception {
		...
		if (!jo.has("tag")) return is;
		JSONObject jo1 = jo.getJSONObject("tag");
		if (jo1.length() == 0) return is;
		Object tag = null;
