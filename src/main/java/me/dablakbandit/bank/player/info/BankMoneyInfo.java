...
public class BankMoneyInfo extends IBankInfo implements JSONInfo, BankDefaultInfo, PermissionsInfo {
	...
	public boolean withdrawMoney(CorePlayers pl, double withdraw) {
		...
		double actualWithdraw = calculator.getCalculation();
		double tax = calculator.getTax();

		boolean complete = withdrawMoney(pl.getPlayer().getName(), withdraw, actualWithdraw);
		if (complete) {
			BankLanguageConfiguration.sendFormattedMessage(pl, BankLanguageConfiguration.MESSAGE_MONEY_WITHDRAW.get().replaceAll("<money>", Format.formatMoney(actualWithdraw)).replaceAll("<tax>", Format.formatMoney(tax)));

			// Record the transaction
			recordWithdrawTransaction(actualWithdraw);

			// Record tax transaction if applicable
			if (tax > 0) {
				recordTaxTransaction(tax, "Withdrawal tax");
			}
		} else {
	...
	public void deposit(CommandSender sender, CorePlayers pl, double amount) {
		...
		double actualDeposit = calculator.getCalculation();
		double tax = calculator.getTax();

		if (amount == 0.0 || depositMoney(pl.getPlayer().getName(), amount, tax)) {
			if (amount != 0.0) {
				BankLanguageConfiguration.sendFormattedMessage(sender, BankLanguageConfiguration.MESSAGE_MONEY_DEPOSIT.get().replaceAll("<money>", Format.formatMoney(calculator.getCalculation())).replaceAll("<tax>", Format.formatMoney(tax)));

				// Record the deposit transaction
				recordDepositTransaction(actualDeposit);

				// Record tax transaction if applicable
				if (tax > 0) {
					recordTaxTransaction(tax, "Deposit tax");
				}
			}
	...
	// New methods to record transactions
	private void recordDepositTransaction(double amount) {
		if (pl.getInfo(BankInfo.class) != null) {
			Transaction transaction = new Transaction(TransactionType.DEPOSIT, amount, TransactionDescription.MONEY_DEPOSIT);
			pl.getInfo(BankInfo.class).getMoneyTransactionInfo().addTransaction(transaction);
		}
	}

	private void recordWithdrawTransaction(double amount) {
		if (pl.getInfo(BankInfo.class) != null) {
			Transaction transaction = new Transaction(TransactionType.WITHDRAW, amount, TransactionDescription.MONEY_WITHDRAWAL);
			pl.getInfo(BankInfo.class).getMoneyTransactionInfo().addTransaction(transaction);
		}
	}
	...
	private void recordSendTransaction(double amount, String recipient) {
		if (pl.getInfo(BankInfo.class) != null) {
			Transaction transaction = new Transaction(TransactionType.SEND, amount, TransactionDescription.MONEY_SEND_TO, recipient);
			pl.getInfo(BankInfo.class).getMoneyTransactionInfo().addTransaction(transaction);
		}
	}
	...
	public void recordReceiveTransaction(double amount, String sender) {
		if (pl.getInfo(BankInfo.class) != null) {
			Transaction transaction = new Transaction(TransactionType.RECEIVE, amount, TransactionDescription.MONEY_RECEIVE_FROM, sender);
			pl.getInfo(BankInfo.class).getMoneyTransactionInfo().addTransaction(transaction);
		}
	}
	...
	private void recordInterestTransaction(double amount) {
		if (pl.getInfo(BankInfo.class) != null) {
			Transaction transaction = new Transaction(TransactionType.INTEREST, amount, me.dablakbandit.bank.player.info.transaction.TransactionDescription.MONEY_INTEREST);
			pl.getInfo(BankInfo.class).getMoneyTransactionInfo().addTransaction(transaction);
		}
	}
	...
	private void recordTaxTransaction(double amount, String description) {
		if (pl.getInfo(BankInfo.class) != null) {
			Transaction transaction = new Transaction(TransactionType.TAX, amount, me.dablakbandit.bank.player.info.transaction.TransactionDescription.MONEY_TAX, description);
			pl.getInfo(BankInfo.class).getMoneyTransactionInfo().addTransaction(transaction);
		}
	}
