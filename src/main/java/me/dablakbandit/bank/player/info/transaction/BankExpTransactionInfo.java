...
public class BankExpTransactionInfo extends IBankInfo implements JSONInfo {
	...
	public BankExpTransactionInfo(CorePlayers pl) {
		super(pl);
	}

	public void addTransaction(Transaction transaction) {
		transactions.add(0, transaction);
		int maxTransactions = BankPluginConfiguration.BANK_EXP_TRANSACTION_MAX_HISTORY.get();
		...
		LoaderManager.getInstance().runAsync(() -> {
			BankDatabaseManager.getInstance().getInfoDatabase().getExpTransactionDatabase().insertTransaction(pl.getUUIDString(), transaction.getType().name(), transaction.getAmount(), transaction.getDescription().name(), transaction.getDescriptionParam());
		});
	}
