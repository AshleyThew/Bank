...
public class BankPermissionConfiguration extends CommentAdvancedConfiguration {
	...
	@CommentArray({"Give a player the permission: bank.loan.interest.0.05 for 5% interest", "Lowest value will be taken from the user bank.loan.interest.0.05 & bank.loan.interest.0.01 will select 1% interest"})
	public static final BankDoublePermissionPath PERMISSION_LOAN_INTEREST = new BankDoublePermissionPath("bank.loan.interest.");

	@CommentArray({"Give a player the permission: bank.loan.amount.1000.5 for a $1000.50 maximum loans", "Use main config to enable combined totals, bank.loan.amount.1000.5 & bank.loan.amount.10.5 would be a total of $1011.00"})
	public static final BankDoublePermissionPath PERMISSION_LOAN_AMOUNT = new BankDoublePermissionPath("bank.loan.amount.");

	@CommentArray({"Give a player the permission: bank.item.slots.1000 for a 1000 slots", "Use main config to enable combined totals, bank.item.slots.1000 & bank.item.slots.10 would be a total of 1010 slots"})
	...
	@Comment("Permission to create cheques")
	public static final PermissionPath PERMISSION_CHEQUE_CREATE = new BankPermissionPath("bank.cheque.create", true);
	@Comment("Permission to get cheque books")
	public static final PermissionPath PERMISSION_CHEQUE_BOOK = new BankPermissionPath("bank.cheque.book", true);
	...
	public static final BankPermissionStringListPath PERMISSION_ADMIN_PERMISSIONS = new BankPermissionStringListPath("bank.item.slots.9");
	...
	@CommentArray({"Give a player the permission: bank.money.interest.online.0.05 for 5% interest", "Highest value will be taken from the user bank.money.interest.online.0.05 & bank.money.interest.online.0.01 will select 5% interest"})
	...
	@CommentArray({"Give a player the permission: bank.money.interest.offline.0.05 for 5% interest", "Highest value will be taken from the user bank.money.interest.offline.0.05 & bank.money.interest.offline.0.01 will select 5% interest"})
	...
	@CommentArray({"Give a player the permission: bank.exp.interest.online.0.05 for 5% interest", "Highest value will be taken from the user bank.exp.interest.online.0.05 & bank.exp.interest.online.0.01 will select 5% interest"})
	...
	@CommentArray({"Give a player the permission: bank.exp.interest.offline.0.05 for 5% interest", "Highest value will be taken from the user bank.exp.interest.offline.0.05 & bank.exp.interest.offline.0.01 will select 5% interest"})
