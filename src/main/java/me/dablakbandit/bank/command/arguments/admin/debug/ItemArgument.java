...
public class ItemArgument extends BankEndArgument {
			...
			try {
				...
				if (result.get("success").getAsBoolean()) {
					...
					Bukkit.getScheduler().runTask(BankPlugin.getInstance(), () -> {
						JSONFormatter formatter = new JSONFormatter().appendHoverClick(BankLanguageConfiguration.BANK_ADMIN_DEBUG_ITEM_URL.get().replaceAll("<url>", logUrl), new ShowTextEvent(ChatColor.GREEN + "Click to open"), new OpenUrlEvent(logUrl));
	...
	private String toString(ItemStack item) throws Exception {
