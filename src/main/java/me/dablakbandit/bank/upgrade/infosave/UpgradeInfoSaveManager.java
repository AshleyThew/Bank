...
public class UpgradeInfoSaveManager {
	...
	public void upgrade(IInfoDatabase infoDatabase) {
		if (infoDatabase instanceof SQLInfoDatabase) {
			upgradeSql((SQLInfoDatabase) infoDatabase);
		}
	}

	private void upgradeSql(SQLInfoDatabase infoDatabase) {
		if (!infoDatabase.tableExists(infoDatabase.getDatabase().getConnection(), "bank_info_type")) {
			return;
		}
