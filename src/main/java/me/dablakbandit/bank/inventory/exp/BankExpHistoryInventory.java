...
public class BankExpHistoryInventory extends BankInventoryHandler<BankExpInfo> {
    ...
    protected boolean open(CorePlayers pl, Player player, int size, String title) {
        ...
        if (bankInfo != null) {
            bankInfo.getExpTransactionInfo().setScrolled(0);
        }
        return super.open(pl, player, size, title);
    }

    private void addInfo() {
        ItemStack is = BankItemConfiguration.BANK_EXP_BLANK.get();
        for (int i = 0; i < 9; i++) {
            setItem(i, () -> is);
        }
    }
    ...
    private void addScrolls() {
        setItem(BankItemConfiguration.BANK_EXP_HISTORY_SCROLL_UP, (pl) -> addScroll(pl, -1));
        setItem(BankItemConfiguration.BANK_EXP_HISTORY_SCROLL_DOWN, (pl) -> addScroll(pl, 1));
    }
    ...
    private void addScroll(CorePlayers pl, int add) {
        BankInfo bankInfo = pl.getInfo(BankInfo.class);
        if (bankInfo != null) {
            BankExpTransactionInfo txInfo = bankInfo.getExpTransactionInfo();
    ...
    private void addTransactions(int size) {
        int show = BankItemConfiguration.BANK_EXP_HISTORY_LIST.getExtendValue("Slots", Integer.class);
        int start = BankItemConfiguration.BANK_EXP_HISTORY_LIST.getExtendValue("Start", Integer.class);
    ...
    private ItemStack getTransactionItemStack(int slot, BankExpInfo info) {
        ...
        String amount = Format.formatExp(tx.getAmount());
    ...
    private ItemStack getTransactionTypeItem(TransactionType type) {
        switch (type) {
            case DEPOSIT:
                return BankItemConfiguration.BANK_EXP_HISTORY_DEPOSIT.get();
            case WITHDRAW:
                return BankItemConfiguration.BANK_EXP_HISTORY_WITHDRAW.get();
            case SEND:
                return BankItemConfiguration.BANK_EXP_HISTORY_SEND.get();
            case RECEIVE:
                return BankItemConfiguration.BANK_EXP_HISTORY_RECEIVE.get();
            case INTEREST:
                return BankItemConfiguration.BANK_EXP_HISTORY_INTEREST.get();
            case TAX:
                return BankItemConfiguration.BANK_EXP_HISTORY_TAX.get();
            default:
