...
public class BankItemDefaultInventory extends BankInventoryHandler<CorePlayers> implements BankAdminInventory {
	...
	public void onClick(CorePlayers pl, int slot, InventoryClickEvent event) {
		if (slot >= implementation.getDefault().size()) {
			return;
		}
	...
	private ItemStack getItemStack(int slot) {
		...
		if (get >= implementation.getDefault().size()) {
			return null;
		}
