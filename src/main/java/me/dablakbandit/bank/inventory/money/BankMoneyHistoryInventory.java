...
public class BankMoneyHistoryInventory extends BankInventoryHandler<BankMoneyInfo> {
	...
	private ItemStack getTransactionItemStack(int slot, BankMoneyInfo info) {
		...
		String dateFormat = BankLanguageConfiguration.MONEY_HISTORY_DATE_FORMAT.get();
	   ...
	   private String formatTimeDelta(long deltaMillis) {
		   ...
		   if (days > 0) {
			   String format = BankLanguageConfiguration.MONEY_HISTORY_DELTA_DAYS.get();
		   ...
		   if (hours > 0) {
			   String format = BankLanguageConfiguration.MONEY_HISTORY_DELTA_HOURS.get();
			   return format.replace("<days>", String.valueOf(days))
							.replace("<hours>", String.valueOf(hours))
							.replace("<minutes>", String.valueOf(remMinutes));
		   }
		   String format = BankLanguageConfiguration.MONEY_HISTORY_DELTA_MINUTES.get();
		   return format.replace("<days>", String.valueOf(days))
						.replace("<hours>", String.valueOf(hours))
