...
public class Economy_Bank extends AbstractEconomy{
	...
	public EconomyResponse withdrawPlayer(String name, double amount){
		...
		String uuid = API.getUUID(name);
		if(uuid == null){ return new EconomyResponse(0.0D, 0.0D, EconomyResponse.ResponseType.FAILURE, "Unknown bank player " + name); }
		if(!API.isOnline(uuid) && API.isLocked(uuid)){ return new EconomyResponse(0.0D, 0.0D, EconomyResponse.ResponseType.FAILURE, "Bank player " + name + " is locked"); }
		double has = API.getMoney(uuid);
		if(has < amount){ return new EconomyResponse(0.0D, 0.0D, EconomyResponse.ResponseType.FAILURE, "Not enough balance"); }
		has -= amount;
	...
	public EconomyResponse depositPlayer(String name, double amount){
		...
		double has = API.getMoney(uuid);
		has += amount;
		if(has <= 0.0){ return new EconomyResponse(0.0D, 0.0D, EconomyResponse.ResponseType.FAILURE, "Invalid bank transaction"); }
		API.setMoney(uuid, has);
		return new EconomyResponse(amount, has, EconomyResponse.ResponseType.SUCCESS, null);
	}
